...
// make sure the persistent folder of user files exists
// TODO if using modulus, replace __dirname with process.env.CLOUD_DIR
var folder = __dirname + "/users/";
...
// TODO replace url with the correct url for your 
...
// TODO for testing that users are setup correctly, replace username with that of a known user
...
app.configure(function() {
  ...
  // app.use(express.session({ secret: 'mighty ducks', store: sessionStore, key: 'hello.sid'})); 
  // CHANGE THIS SECRET!
  app.use(express.cookieSession({secret: 'mighty ducks'}));
...
app.post('/forgot-pwd', function(req, res){
    ...
    User.findOne({email: req.body.email}, function(err, existingUser){
        ...
        if (existingUser) {
            ...
            // TODO update the system emails
            existingUser.save(function(err){
                if (err) {throw err;} else {
                    // email newPassword to user
                    var mailOptions = {
                        from: "PandoLab Admin <admin@your-site.com>",
                        to: req.body.email,
                        subject: "Password Reset - PandoLab",
                        text: "Your password has been reset. Your new password is " + newPassword + ". You may change this once you have logged in.\n\n- Pando Lab Admin (admin@your-site.com)",
                        html: "<h1>Password Reset - PandoLab</h1> <p>Your password has been reset. Your new password is <strong>" + newPassword + "</strong>. You may change this once you have logged in.</p> - Pando Lab Admin (<a href='mailto:admin@your-site.com'>admin@your-site.com</a>)"
                    };
...
app.get('/manager', ensureAuthenticated, function(req, res){
    ...
    if (activeSessions.hasOwnProperty(req.user.email)) {
        ...
        // TODO replace __dirname with process.env.CLOUD_DIR if hosting on modulus
        var folder = __dirname + "/users/" + req.user.email.replace(/[\.@]/g, "_") + "/";
        //var folder = process.env.CLOUD_DIR + "/users/" + req.user.email.replace(/[\.@]/g, "_") + "/";
...
app.post('/upload-zip', ensureAuthenticated, function(req, res) {
    ...
    if(file.type === "application/zip") {
        ...
        // TODO change __dirname to process.env.CLOUD_DIR if hosting on modulus
        fs.readdir(__dirname + "/users/" + userFolder + "/treatments/", function(err, files){
        //fs.readdir(process.env.CLOUD_DIR + "/users/" + userFolder + "/treatments/", function(err, files){
            console.log(files);
            if (err) { throw(err); }
            ...
            //var targetPath = process.env.CLOUD_DIR + "/users/" + userFolder + "/treatments/" + dirName;
